...
The database is built on **PostgreSQL 16**. The schema covers members, ministries, services, visitors, users, giving, attendance, and prayer requests. Tables are linked via foreign keys where appropriate.
...
Stores recurring service templates used to populate the public service schedule and calendar. Each row represents a service type (e.g. Sunday Main Service), not an individual occurrence. Actual dated occurrences are generated or managed at the application layer.
...
| `preacher_id` | `UUID` | `FK → members(id), NULLABLE` | Member assigned as preacher for this service |
| `leader_id` | `UUID` | `FK → members(id), NULLABLE` | Member assigned as service leader / worship leader |
| `special_date` | `DATE` | `NULLABLE` | Specific date for non-recurring (special) services |
| `is_active` | `BOOLEAN` | `DEFAULT TRUE` | Whether this service is currently active / displayed |
| `created_at` | `TIMESTAMPTZ` | `DEFAULT NOW()` | Record creation timestamp |
| `updated_at` | `TIMESTAMPTZ` | `DEFAULT NOW()` | Last updated timestamp |
...
- `idx_services_preacher` on `preacher_id`
- `idx_services_leader` on `leader_id`
...
erDiagram
    ...
    members ||--o{ services : "preaches at"
    members ||--o{ services : "leads"
...
- Service schedule management — add/edit recurring and special service templates
...
| Service Calendar (Full) | Calendar view driven by services table | 🟢 Low | Phase 3 |
...
*Confidential — Internal Use Only | Version 1.3 | June 2026*
